...
from clients import *
import statistics
from malicious_cordinator import MaliciousCordinator
...
class Server():
    def __init__(self, model, dataLoader, criterion=F.nll_loss, device='cpu', minimumIterationToStartTheAttack=5):
        self.clients = []
        self.clientsNumOfBeingMalicious = []
        self.model = model
        ...
        self.minimumIterationToStartTheAttack = minimumIterationToStartTheAttack
        MaliciousCordinator.minimumIterationToStartTheAttack = minimumIterationToStartTheAttack
        MaliciousCordinator.model = model

        self.lastModelAccuracy = 0
    ...
    def attach(self, c):
        ...
        self.clientsNumOfBeingMalicious.append(0)
    ...
    def test(self):
        ...
        if self.isSaveChanges:
            ...
            self.saveTestingResults(testingData)
    ...
    def train(self, group):
        ...
        MaliciousCordinator.attackerNodeBeforeAttackGradient = {}
        MaliciousCordinator.attackerNodeAvgDelta = {}
        MaliciousCordinator.attackerNodeSumDelta = {}
        MaliciousCordinator.attackerNodehogAvgLen = {}
        ...
        MaliciousCordinator.attackerNodes = []
        MaliciousCordinator.numberOfAttackerNodes = 0
        MaliciousCordinator.numberOfClientsTraining = len(selectedClients)
        MaliciousCordinator.currentIteration = self.iter
        MaliciousCordinator.attackerNodeChoosenGradientInThisIteration = {}
        MaliciousCordinator.attackerNodeChoosenAvgDeltaInThisIteration = {}
        MaliciousCordinator.attackerNodeChoosenSumDeltaInThisIteration = {}
        MaliciousCordinator.attackerNodeChoosenhogAvgLenInThisIteration = {}
        MaliciousCordinator.attackerNodeOfLabelFlipping = []
        MaliciousCordinator.attackerNodeOfBackdoor = []
        MaliciousCordinator.attackerNodeOfMultiLabelFlipping = []
        MaliciousCordinator.lastModelAccuracy = self.lastModelAccuracy

        # for index, c in enumerate(selectedClients):
        #     c.train()
        #     c.update()

        streams = [torch.cuda.Stream() for _ in range(len(selectedClients))]
        ...
        if self.isSaveChanges:
            self.saveChanges(selectedClients, "trained_clients_before_aggregation")
        ...
        if self.isSaveChanges:
            self.saveModel("aggregated_model")

        self.iter += 1
    
    def saveLabels(self, clients, prefix):
        savepath = f'{self.savePath}/{prefix}_labels_{self.iter}.pt'
        labels = torch.ones(len(clients))
        for index, client in enumerate(clients):
            labels[index] = client.cid
        torch.save(labels, savepath)

    def saveIterationClientsInfo(self, clientsMalicoiusInfoData):
        savepath = f'{self.savePath}/iteration_malicoius_clients_info_{self.iter}.pt'
        torch.save(clientsMalicoiusInfoData, savepath)

    def saveModel(self, prefix):
        savepath = f'{self.savePath}/{prefix}_{self.iter}.pth'
        torch.save(self.model.state_dict(), savepath)

    def saveTestingResults(self, testingData):
        savepath = f'{self.savePath}/testing_results_{self.iter-1}.pt'
        torch.save(testingData, savepath)

    def saveChanges(self, clients, prefix):

        Delta = deepcopy(self.emptyStates)
        deltas = [c.getDelta() for c in clients]

        self.saveLabels(clients, prefix)
...
#             return
        if saveOriginal:
            savepath = f'{self.savePath}/{prefix}_values_{self.iter}.pt'

            torch.save(deltas, savepath)
    ...
    def add_mal_id(self, sus_flip_sign, sus_uAtk, sus_tAtk):
        ...
        logging.info("mal_ids={}".format(self.mal_ids))
    ...
    def mud_hog(self, clients):
        ...
        if self.iter >= self.tao_0:
            ...
            # print("mud hug calcalated median for sign flipping", median_sHoG)
            ...
            # print("mud hug calcalated median for untargetted", normal_cent)
            ...
            # print("mud hug calcalated median for unreliable", median_sHoG)
            ...
            if self.isSaveChanges:
                self.saveChanges(normal_clients, "selected_clients_before_aggregation")
                clientsMalicoiusInfoData = {
                    "mal_ids": self.mal_ids,
                    "tAtk_id": tAtk_id,
                    "uAtk_id": uAtk_id,
                    "normal_client": normal_clients,
                    "len_mal_ids": len(self.mal_ids),
                    "len_tAtk_id": len(tAtk_id),
                    "len_uAtk_id": len(uAtk_id),
                    "len_normal_client": len(normal_clients),
                }
                self.saveIterationClientsInfo(clientsMalicoiusInfoData)
        else:
    ...
    def FedFuncWholeNet(self, clients, func):
        ...
        print("clients participating in delta", [obj.cid for obj in clients])
        # mean_values = {key: values.mean().item() for key, values in Delta.items()}
        # print("delta", mean_values)
        return Delta
